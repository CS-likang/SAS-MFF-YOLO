...
def test(data,
    ...
    if dataloader is None:  # not training
        ...
        dataloader = create_dataloader(path, imgsz, batch_size, int(max(model.stride)), opt,
                                       hyp=None, augment=False, cache=False, pad=0.5, rect=True, is_rect=False)[0]
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        # TODO Plot images
        if batch_i < 3:
            f = 'visual_gt_pre/test_batch%g_gt.jpg' % batch_i  # filename
            plot_images(img, targets, paths, f, names)  # ground truth
            f = 'visual_gt_pre/test_batch%g_pred.jpg' % batch_i
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data', type=str, default='data/voc_my.yaml', help='*.data path')
