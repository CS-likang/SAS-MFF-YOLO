...
## 每个Head一共(80个类 + 1个概率 + 4坐标) * 3锚定框，一共255个channels。
class Detect(nn.Module):
    def __init__(self, nc=80, anchors=()):  # detection layer
        super(Detect, self).__init__()
        self.stride = None  # strides computed during build
        self.nc = nc  # number of classes = 检测类别数量
        self.no = nc + 5  # number of outputs per anchor = 每个anchor需要输出的通道
        self.nl = len(anchors)  # number of detection layers = 3
        self.na = len(anchors[0]) // 2  # number of anchors = 该层特征需要检测输出的anchor数量
...
class Model(nn.Module):
    def __init__(self, model_cfg='yolov5s.yaml', ch=3, nc=None):  # model, input channels, number of classes
        ...
        if isinstance(m, Detect):
            ...
            # m.stride = torch.tensor([s / x.shape[-2] for x in self.forward(torch.zeros(2, ch, s, s))])  # forward TODO 7.23修改完成
            m.stride = torch.tensor([s / x.shape[-2] for x in self.forward(torch.zeros(2, ch, s, s))[0]])  # forward TODO 10.03修改完成
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='my_yolo.yaml', help='model.yaml')
